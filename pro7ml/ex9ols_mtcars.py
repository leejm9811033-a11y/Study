...
# x : hp(마력수), y : mpg(연비)
# print(mtcars.corr())
print(np.corrcoef(mtcars.hp, mtcars.mpg))   # 음의 관계 (trade off)
print(np.corrcoef(mtcars.wt, mtcars.mpg))   # 음의 관계 (trade off)
...
print('마력수 110에 대한 연비 예측 값 : ', -0.0682 * 110 + 30.0989)     # 22.5969
print('마력수 110에 대한 연비 예측 값 : ', \
        result.predict(pd.DataFrame({'hp':[110]})).values)     # 22.596
...
print('마력수 110 + 무게 5에 대한 연비 예측값 : ', \
        result2.predict(pd.DataFrame({'hp':[110], 'wt':[5]})).values)    # 14.3403

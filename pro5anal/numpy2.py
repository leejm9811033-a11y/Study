...
print()
print(x + y)        # 파이썬 연산자 또는 함수 (속도가 상대적으로 느림)
print(np.add(x, y)) # 넘파이 함수   (universal function) (상대적으로 빠름)
...
print(np.dot(v, w)) # 9 * 11 + 10 * 12는 약1.4도다.
print(np.dot(x, v))
...
print(np.argmax(x), ' ', np.argmin(x))  # index를 반환
...
print('\n전치(Transpose) - 2차원 배열 행과 열의 위치를 변환')
print(x)
print(x.T)
print(x.transpose())

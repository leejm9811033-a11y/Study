...
print('--- concat ---')
pd.concat([df1, df3], axis=0)   # 행 단위
pd.concat([df1, df3], axis=1)   # 열 단위
...
print()
print(df.set_index(['city', 'year'])) # set_index : 기존 행 인덱스를 제거하고 첫번째 열 인덱스
print()
...
print(df.pivot_table(values='pop', index=['year'], columns=['city'], margins=False))
print()
hap = df.groupby(['city'])
print(hap)
print(hap.sum())

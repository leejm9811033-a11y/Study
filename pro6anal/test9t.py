...
two_sample = stats.ttest_ind(male, female, equal_var=True)  # 위와 동일. 두 집단의 분산
print(two_sample)
tv, pv = two_sample
print('t 검정통계량 : ', tv)   # 1.233193127514512
print('p 값 : ', pv)   # 0.2525076844853278
# 해석 : p=value 0.2525 > 0.5  귀무 채택. 통계적으로 유의하지 않다.

print('----------------')
# 선행 조건 1) 두 집단이 각각 정규분포를 따라야 한다.
print('male 의 정규성 : ', stats.shapiro(male))       # p 0.600371 > 0.05 정규성 만족
...
# bartlett   정규성이 있을 때만 사용, 이상치에 덜 민감, 정규분포일 때만 사용

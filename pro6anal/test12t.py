# Paired Samples t-test(대응표본 t검정, 동일집단표본 t검정)
# 하나의 집단에 대해 독립변수를 적용하기 전과 후의 종속변수(평균)의 수준을 측정하고
# 이들의 차이가 통계적으로 유의한지 분석
# 동일한 관찰 대상으로 처리 이전과 이후를 1:1 대응시킨 검정
# 집단 간 비교가 아니므로 등분산 가정을 할 필요는 없다.
# 예 : 광고 전후의 상품 선호도 측정, 투자 대비 상품 판매량 ....

# 실습 1 : 3강의실 학생들을 대상으로 특강 시험 점수에 영향을 주었는가?
...
sns.displot(x1, kde=False)
sns.displot(x2, kde=False)
plt.show()
